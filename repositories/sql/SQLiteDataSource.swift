...
public class SQLiteDataSource<K:Hashable, T:Cacheable where T.K == K> : BaseDataSource<K,T> {
    ...
    public convenience init(version: Int, ttl: NSTimeInterval, dbName: String) {
        self.init(
            version: version,
            dbPath: generateDBPath(dbName),
            policies: [
    ...
    convenience init(version: Int, ttl: NSTimeInterval, sqliteRuntimeConfiguration: SQLiteRuntimeConfiguration) {
        self.init(
            version: version,
            dbPath: generateDBPath(),
            policies: [
                ...
                try self.executeSQLBlock(db, fromInsideTransaction: true) { db in
                    ...
                    if result.next() {
                        if  let cachedItem = self.cachedItemFromDBResult(result),
                            item = cachedItem.item where self.itemIsValid(cachedItem, forCachePolicies: self.policies) {
    ...
    public override func getPage(offset offset: Int, limit: Int) -> Promise<CollectionContainer<T>> {
        return Promise {
            (fulfill, reject) in

            let collectionContainer: CollectionContainer<T>? = self.getPage(offset: offset, limit: limit)
            if let collectionContainer = collectionContainer {
                fulfill(collectionContainer)
            } else {
                reject(self.noItemsInRepositoryError)
            }
        }
    ...
    override public func getPage(offset offset: Int, limit: Int) -> CollectionContainer<T>?  {
        guard tableExists() else {
            return nil
        }

        let requestedItemsCount = offset + limit
        let cachedItemsCount = getCount()

        // If we don't have enough cached items to fulfill the request and we know that there are more items
        // we should fail and let other DataSource try to fetch them all
        guard cachedItemsCount >= requestedItemsCount || lastOffsetWithMoreItems == NoMoreItemsOffset else {
            return nil
        }

        let sql = "SELECT * from \(tableName) LIMIT \(limit) OFFSET \(offset);"
        var collection: CollectionContainer<T>? = nil
        queue.inTransaction { db, rollback in
            do {
                try self.executeSQLBlock(db, fromInsideTransaction: true) { db in
                    var cachedItems: [CacheItem<T>] = []
                    ...
                    if filteredItems.count != cachedItems.count {
                        ...
                        let hasMore = (requestedItemsCount <= self.lastOffsetWithMoreItems) || (requestedItemsCount < cachedItemsCount)

                        // Return the unwrapped items
                        collection = CollectionContainer(
                            items: self.unwrapCachedItems(filteredItems),
